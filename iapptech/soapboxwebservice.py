...
import re
import base64
from soapbox.models import SoapboxAppusers ,SoapboxDevicetoken,SoapboxCategories,SoapboxPostlistens,SoapboxTagrelatedtopost,SoapboxConnectiontablesocialid,SoapboxSubscribechannel,SoapboxPosts,SoapboxPostlikeandshare,SoapboxUserfollowers
...
def userPostList(request):
	...
	if(headervalue == 0):
		...
		try: 
			...
			userPostInfo=SoapboxTagrelatedtopost.objects.all().filter(user_id=userid)
...
def getPopularPostList(request):
	...
	if(headervalue == 0):
		...
		try:
			...
			postlike = "SELECT id,post_id as postid, user_id as userid, sum(num_share) as total ,sum( num_likes ) as likes ,sum(num_listen) as listen FROM soapbox_postlikeandshare WHERE user_id ="+str(userid)+" GROUP BY post_id ORDER BY total DESC , likes DESC;"
			...
			for postlikes in SoapboxPostlikeandshare.objects.raw(postlike):
				...
				postArray['profilepic']=chkusr.profilepic
				postArray['comment_count']=0
				if(postlikes.total == None):
					postArray['share_count']=0
				else:
					postArray['share_count']=int(postlikes.total)
				if(postlikes.likes == None):
					postArray['like_count']=0
				else:
					postArray['like_count']=int(postlikes.likes)
		...
		try:
			...
			if(userid !='' and post_id!=''and other_reply_type!='' ):
				if(other_reply_type == 1):
					...
					return HttpResponse(json.dumps({"success":0,"message": "Other_reply_type is incorrect."}))
