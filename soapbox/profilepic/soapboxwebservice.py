	...
	try:
		...
		if(pwd != '' and usertype ==0 and socialid == ''):
			...
			try:
				...
				userSave=SoapboxAppusers(apikey=apikey,usertype='appuser',phonenumber=phonenumber,firstname=firstname,lastname=lastname,username=username,email=email,password=md5.new(pwd).hexdigest(),dateofbirth=dateofbirth,gender=gender,created_at=nowTime,profilepic=imageurl)
				userSave.save()
				user_id=userSave.pk
			if(devicetoken!='' and devicetype!=''):	
				...
				userInfo={'site_name':'appuser','phonenumber':phonenumber,'apikey':apikey,'userid':user_id,'firstname':firstname,'lastname':lastname,'username':username,'email':email,'dateofbirth':dateofbirth,'gender':gender,'profilepicurl':imageurl}
				return HttpResponse(json.dumps({'message':'Registered successfully with Soapbox App.','success':1,'information':userInfo}))
			else:
				userInfo={'phonenumber':phonenumber,'site_name':'appuser','apikey':apikey,'userid':user_id,'firstname':firstname,'lastname':lastname,'username':username,'email':email,'dateofbirth':dateofbirth,'gender':gender,'profilepicurl':imageurl}
				return HttpResponse(json.dumps({'message':'Registered successfully with Soapbox App.','success':1,'information':userInfo}))
			...
			try:
				...
				updateValue ={"phonenumber":phonenumber,"firstname": firstname,"lastname": lastname,"username":username,"email":email,"dateofbirth":dateofbirth,"gender":gender,"profilepic":imageurl}
