...
import re
import base64
from soapbox.models import SoapboxAppusers ,SoapboxDevicetoken,SoapboxCategories,SoapboxPostlikes,SoapboxTagrelatedtopost,SoapboxConnectiontablesocialid,SoapboxSubscribechannel,SoapboxComments,SoapboxPosts,SoapboxUserfollowers,SoapboxPostlikeandshare,SoapboxPostlistens,SoapboxPostshares
		...
		try: 
			...
			if userPostInfo.exists():
				for userPostInfos in userPostInfo.all():
					...
					postlike="SELECT id,sum(num_likes) as likes, sum(num_share) as share, sum(num_listen) as listen  FROM soapbox_postlikeandshare where post_id="+str(userPostInfos.id)+";"
			...
			if(friend_list == ''):
				...
				if friendInfo.exists():
					for friendInfos in friendInfo.all():
						recFriendArray = {}
						recFriendArray['user_id']=friendInfos.id
			...
			if(user_id == ''):
				...
				if followingpostlist.exists():
					for followingpostlists in followingpostlist.all(): 
						...
						postlike="SELECT id,sum(num_likes) as likes, sum(num_listen) as listen, sum(num_share) as share  FROM soapbox_postlikeandshare where post_id="+str(followingpostlists.id)+";"
						for postlikes in SoapboxPostlikeandshare.objects.raw(postlike):
    ...
    if(headervalue == 0):
        ...
        try:
            ...
            if(userid == ''):
            	...
            	postlike = "select *,count(num_share)as total_share,count(num_likes)as total_likes,count(num_listen)as listen_count,soapbox_tagrelatedtopost.id as postid ,soapbox_tagrelatedtopost.user_id as userid    from  soapbox_tagrelatedtopost left join  soapbox_postlikeandshare on  soapbox_postlikeandshare.post_id=soapbox_tagrelatedtopost.id  where  soapbox_tagrelatedtopost.user_id =42 group by soapbox_tagrelatedtopost.id order by total_share DESC ,listen_count DESC, total_likes DESC"
