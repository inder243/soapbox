		...
		try:
			...
			if(channelname!=''):
				...
				if channelInfo.exists():
					...
					rearray=[]
					return HttpResponse(json.dumps({'message':'No Result Found.','success':0,'channel_name':rearray}))
			else:
				error={'error':'invalid parameters'}
				return HttpResponse(json.dumps(error))
...
def connectSocialMedia(request):
	...
	if(headervalue == 0):
		...
		try:
			...
			socialsitename = connectSocialMedia_json_data['socialsite_name']
			if(userid == ''):
				return HttpResponse(json.dumps({'message':'userid is Missing.','success':0}))
			if(socialid == ''):
				return HttpResponse(json.dumps({'message':'socialid is Missing.','success':0}))
			if(socialsitename == ''):
				return HttpResponse(json.dumps({'message':'socialsite_name is Missing.','success':0}))
...
def subscribechannel(request):
	...
	if(headervalue == 0):
		...
		try:
			...
			if(issubscribe == ''):
				return HttpResponse(json.dumps({'message':'is_subscribe flag is Missing.','success':0}))
	...
	if(headervalue == 0):
		...
		try:
			...
			for item in jsonData:
				...
				channelinfos = SoapboxCategories.objects.all().filter(channel_name= item.get("hash_tag_name"))
				
				if channelinfos.exists():
					post=SoapboxPosts(userid=userid,post_file_url=audiofile,posted_on=nowTime,posted_at=posted_at,channelid=item.get("id"),channel_name=item.get("hash_tag_name"))
					post.save()
					post_id = str(post.pk)
					recArray={"id":post_id,"post_file_url":audiofile,"posted_on":posted_on,"posted_at":posted_at,"user_by_id":userid,"hash_tag_id":item.get("id"),"hash_tag_name":item.get("hash_tag_name")}
					records.append(recArray)
					...
					post=SoapboxPosts(userid=userid,post_file_url=audiofile,posted_on=nowTime,posted_at=posted_at,channelid=channel_id,channel_name=item.get("hash_tag_name"))
					post.save()
					post_id = str(post.pk)
					recArray={"id":post_id,"post_file_url":audiofile,"posted_on":posted_on,"posted_at":posted_at,"user_by_id":userid,"hash_tag_id":channel_id,"hash_tag_name":item.get("hash_tag_name")}
					records.append(recArray)
		...
		try: 
			...
			if userPostInfo.exists():
				for userPostInfos in userPostInfo.all(): 
					...
					recArray['post_title']=userPostInfos.channel_name
					...
					hashInfo=SoapboxPosts.objects.all().filter(userid=userid,post_file_url=recArray['post_file_url']).distinct()
