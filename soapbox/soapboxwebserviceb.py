...
import re
import base64
from soapbox.models import SoapboxAppusers ,SoapboxDevicetoken,SoapboxCategories
...
def appRegister(request):
	...
	# try:
		...
		if(pwd != '' and usertype ==0 and socialid == ''):
			...
			try:
				...
				userSave=SoapboxAppusers(apikey=apikey,usertype='appuser',firstname=firstname,lastname=lastname,username=username,email=email,password=md5.new(pwd).hexdigest(),dateofbirth=dateofbirth,gender=gender,created_at=nowTime,profilepic=imageurl,phonenumber=phonenumber)
				userSave.save()
				user_id=userSave.pk
			try:
			...
			userInfo={'site_name':'appuser','apikey':apikey,'phonenumber':phonenumber,'userid':user_id,'firstname':firstname,'lastname':lastname,'username':username,'email':email,'dateofbirth':dateofbirth,'gender':gender,'profilepicurl':imageurl}
			...
			try:
				...
				updateValue ={"firstname": firstname,"phonenumber":phonenumber,"lastname": lastname,"username":username,"email":email,"dateofbirth":dateofbirth,"gender":gender,"profilepic":imageurl}
				...
				userInfo={'phonenumber':phonenumber,'socialsite_name':chkuserinfo.usertype,'apikey':chkuserinfo.apikey,'socialid':socialid,'userid':chkuserinfo.id,'firstname':firstname,'lastname':lastname,'username':username,'email':email,'dateofbirth':dateofbirth,'gender':gender,'profilepicurl':imageurl}
				...
				userSave=SoapboxAppusers(phonenumber=phonenumber,apikey=apikey,socialid=socialid,usertype=socialsite_name,firstname=firstname,lastname=lastname,username=username,email=email,dateofbirth=dateofbirth,gender=gender,created_at=nowTime,profilepic=imageurl)
				userSave.save()
				...
				userInfo={'phonenumber':phonenumber,'dateofbirth':dateofbirth,"socialsite_name":socialsite_name,'apikey':apikey,'socialid':socialid,'userid':user_id,'firstname':firstname,'lastname':lastname,'username':username,'email':email,'dateofbirth':dateofbirth,'gender':gender,'profilepicurl':imageurl}
	...
	#register webservice
	#http://52.11.101.155:8080/appregister/?profilepic=&firstname=&lastname=&username=&email=&dateofbirth=&password=&gender=
...
def appLogin(request):
	try:
		...
		#login webservice 
		#http://52.11.101.155:8080/applogin/?email=&password=&devicetoken=&devicetype=
		

	

	#http://52.11.101.155:8080/appaudiourl/
	...
	try:
		...
		if(userid!='' and channelname!=''):
			...
			if channelInfo.exists():
				...
				return HttpResponse(json.dumps({'message':'stored channel name','channelname':records}))
			else:
